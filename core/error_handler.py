...
class ScanError(HandyOsintException):
    ...
    def __init__(self, message: str, target: str = "", platform: str = "", 
                 details: Optional[Dict] = None):
...
class RateLimitError(HandyOsintException):
    ...
    def __init__(self, message: str, platform: str = "", retry_after: int = 0, 
                 details: Optional[Dict] = None):
...
class ErrorLogEntry:
    ...
    def __init__(self, exception: Exception, severity: ErrorSeverity, 
                 context: Optional[Dict] = None, recovery: RecoveryStrategy = RecoveryStrategy.ABORT):
        self.timestamp = datetime.now().isoformat()
...
class ErrorHandler:
    ...
    def handle_exception(self, exception: Exception, context: Optional[Dict] = None,
                        ...
                        recovery: RecoveryStrategy = RecoveryStrategy.ABORT) -> ErrorLogEntry:
        ...
        if severity == ErrorSeverity.INFO:
            ...
            self.error_logger.critical(f"FATAL: {entry.exception_type}: {entry.message}")
        
        return entry
    
    def handle_validation_error(self, message: str, field: str = "",
                               context: Optional[Dict] = None) -> ErrorLogEntry:
    ...
    def handle_database_error(self, message: str, operation: str = "",
                             context: Optional[Dict] = None) -> ErrorLogEntry:
    ...
    def handle_network_error(self, message: str, url: str = "", status_code: int = 0,
                            context: Optional[Dict] = None) -> ErrorLogEntry:
    ...
    def handle_scan_error(self, message: str, target: str = "", platform: str = "",
                         context: Optional[Dict] = None) -> ErrorLogEntry:
        """Handle scan error"""
    ...
    def handle_timeout_error(self, message: str, timeout_seconds: float = 0,
                            context: Optional[Dict] = None) -> ErrorLogEntry:
    ...
    def handle_rate_limit(self, message: str, platform: str = "", retry_after: int = 0,
                         context: Optional[Dict] = None) -> ErrorLogEntry:
    ...
    def safe_call(self, func: Callable, *args, **kwargs) -> Tuple[bool, Any, Optional[str]]:
    ...
    def log_operation(self, operation: str, status: str, 
                     duration: Optional[float] = None, details: Optional[Dict] = None) -> None:
    ...
    def get_error_count(self, severity: Optional[ErrorSeverity] = None) -> int:
        ...
        return sum(1 for entry in self.error_history if entry.severity == severity.value)
